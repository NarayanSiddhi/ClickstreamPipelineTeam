package transform

import org.apache.spark.sql._
import org.apache.spark.sql.functions._
import org.apache.spark.internal.Logging

object ConvertToLowercase extends Logging {
  def convertToLowercase(clickstreamDuplicate:DataFrame,itemsetDuplicate:DataFrame):(DataFrame,DataFrame)={
    try
    { // converting records of redirection_source to lowercase
      val clickstreamLowercase = clickstreamDuplicate.withColumn("redirection_source", lower(col("redirection_source")))

      // converting records of department_name to lowercase
      val itemsetLowercase = itemsetDuplicate.withColumn("department_name", lower(col("department_name")))

      (clickstreamLowercase, itemsetLowercase)
    }
    catch {
      case e: Exception =>
        logInfo("An error occurred during converting to lowercase.", e)
        // Returning original DataFrames as an example
        (clickstreamDuplicate, itemsetDuplicate)
    }
  }
}
